...
try {
    ...
    if (empty($video_url)) {
        echo 'Enter the short TikTok video URL:';
        $video_url = trim(fgets(STDIN));
    }

    $resp = $tiktok->getUnshortenedUrl($video_url);
    if ($resp->isOk() && $resp->isTiktok() && $resp->getTiktok()->isUrl()) {
        echo sprintf('Full TikTok video URL: %s.', $resp->getTiktok()->getUrl());
    }
