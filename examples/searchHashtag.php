...
try {
    ...
    search:
    $resp = $tiktok->searchHashtag($keyword);

    if ($resp->isOk() && $resp->isTiktok() && $resp->getTiktok()->isChallengeList() && count($resp->getTiktok()->getChallengeList()) > 0) {

        echo sprintf('Hashtags found by keyword "%s":', $keyword) . "\n";

        $hashtags = $resp->getTiktok()->getChallengeList();

        foreach ($hashtags as $key => $h) {

            $hashtag = $h->getChallengeInfo()->getChaName();
            $use_count = number_format($h->getChallengeInfo()->getUseCount(), 0, '', ' ');
            $view_count = number_format($h->getChallengeInfo()->getViewCount(), 0, '', ' ');

            echo sprintf('%s. %s | Use Count: %s | View Count: %s', $key + 1, $hashtag, $use_count, $view_count) . "\n";
        }
    } else {
        echo sprintf('Hashtags by keyword "%s" not found.', $keyword) . "\n";
    }
